...
def Browse():
	# Presenting user with a pop-up for directory selection. initialdir argument is optional Retrieving the user-input destination directory and storing it in downloadDirectory
...
def Download():
	...
	# Getting all the available streams of the youtube video and selecting the first from them and downloading the video to destination directory
	getVideo.streams.get_highest_resolution().download(download_Folder)

	# size of video
	# print(getVideo.streams.get_highest_resolution().filesize)

	# Downloading the video to destination directory
	# var_name.download(download_Folder)
